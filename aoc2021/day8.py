...
inputs = ["bgeacd dbfag bcadegf agdce dgfbce bgc bdgca aedcgf bc abec | gcdfbe cbea bc gbc",
...
def solve():
    count = 0
    for line in inputs:
        digits, num = line.split("|")
        digits = list(x for x in digits.split(" ") if x != "")
        num = list(x for x in num.split(" ") if x != "")
...
def prob2():
    ...
    for line in inputs:
        ...
        import itertools
        for possible in itertools.permutations("abcdefg"):
            mapping = {}
            for i, x in enumerate(possible):
                mapping[chr(i + ord('a'))] = x
            ...
            for x in digits:
                ...
                found[source.index(now)] = True

            if good and all(found):
